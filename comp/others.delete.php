...
if (empty($data['i'])) {
    header("Location: ../others.php?m=not_found");
    exit();
}
...
if ($data['type'] == "expenseSubGroup") {
    ...
    if ($stmt->execute()) {
        header("Location: ../others.php?m=group_delete_true");
        exit();
    } else {
        header("Location: ../others.php?m=group_delete_false");
        exit();
    }
    ...
    $stmt->bind_param("i", $data['i']);
    if ($stmt->execute()) {
        header("Location: ../others.php?m=block_delete_true");
        exit();
    } else {
        header("Location: ../others.php?m=block_delete_false");
        exit();
    }
