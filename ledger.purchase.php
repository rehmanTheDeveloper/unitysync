...
$sellers = fetch_Data($conn, $query);

foreach ($sellers as $key => $seller) {
    $fetch_ledger = [
        'type' => "purchase",
        'source' => $seller['acc_id'],
        'pay_to' => $seller['acc_id'],
        'project' => $_SESSION['project']
    ];
    $ledger_obj = new Ledger($PDO_conn);
    $fetched_ledger = $ledger_obj->fetch($fetch_ledger);
    if (!empty($fetched_ledger)) {
        foreach ($fetched_ledger as $single_ledger) {
...
if (!empty($sellers) && !empty($banks)) {
    $a = 0;
    foreach ($sellers as $seller) {
        $accounts[] = $seller;
        $accounts[$a]['type'] = "seller";
        $a++;
    }
                        ...
                        <table class="table table-centered table-hover table-nowrap mb-0 rounded" id="datatable">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th class="border-0 text-end">Cr./Dr.</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end text-success">Rs. <?=number_format($led['credit'])?>
                                    <?=$arrow_up?></td> 
                                    <?php } else { ?>
